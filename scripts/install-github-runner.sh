...
# System packages and pipx CLI tools used by CI workflows (incl. native deps for
# cargo install of s3s-e2e / aws-lc-sys on self-hosted runners).
echo "Installing dependencies (apt: unzip, pipx, build toolchain, cmake)..."
export DEBIAN_FRONTEND=noninteractive
sudo apt-get update -qq
sudo apt-get install -y unzip pipx build-essential cmake pkg-config
