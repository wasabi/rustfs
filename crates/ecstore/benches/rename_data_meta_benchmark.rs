...
use rustfs_filemeta::{ErasureAlgo, FileInfo, FileMeta, S3VersionId};
...
fn bench_rename_data_meta_path(c: &mut Criterion) {
    ...
    for &version_count in VERSION_COUNT_CASES {
        ...
        let replace_version_id = seeded
            ...
            .unwrap_or(S3VersionId::Uuid(Uuid::nil()));
        let replace_version_uuid = match replace_version_id {
        ...
        group.bench_with_input(BenchmarkId::new("read_modify_write", version_count), &version_count, |b, _| {
            b.iter(|| {
                ...
                let search_version_id = Some(replace_version_uuid);
                ...
                if let Some(old_data_dir) = has_old_data_dir {
                    let _ = xlmeta.data.remove_two(replace_version_uuid, old_data_dir);
                }
                let fi = make_file_info(replace_version_uuid, Uuid::new_v4(), 64 * 1024, base_time + Duration::from_millis(1));
        ...
        if let Some(old_data_dir) = prepared.find_unshared_data_dir_for_version(Some(replace_version_uuid)) {
            let _ = prepared.data.remove_two(replace_version_uuid, old_data_dir);
        }
        group.bench_with_input(BenchmarkId::new("add_version_marshal_only", version_count), &version_count, |b, _| {
            b.iter_batched(
                ...
                |mut xlmeta| {
                    let fi =
                        make_file_info(replace_version_uuid, Uuid::new_v4(), 64 * 1024, base_time + Duration::from_millis(1));
                    xlmeta.add_version(fi).expect("add new version");
                    let out = xlmeta.marshal_msg().expect("marshal updated meta");
                    black_box(out);
                },
        ...
        group.bench_with_input(BenchmarkId::new("remove_two_only", version_count), &version_count, |b, _| {
            b.iter(|| {
                let mut xlmeta = FileMeta::load(black_box(&dst_buf)).expect("load dst meta");
                let removed = if let Some(old_data_dir) = xlmeta.find_unshared_data_dir_for_version(Some(replace_version_uuid)) {
                    xlmeta
                        .data
                        .remove_two(replace_version_uuid, old_data_dir)
                        .expect("remove two")
                } else {
                    false
                };
